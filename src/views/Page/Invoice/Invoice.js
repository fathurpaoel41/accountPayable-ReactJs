...
export default class Invoice extends Component {
    ...
    ModalDelete(id,idrequest) {
        this.setState({
            openModalDelete: !this.state.openModalDelete
        });
        console.log("modal Id", id)
        localStorage.setItem("ubahInvoice", id)

        fetch(`http://localhost:8080/api/request/${idrequest}`)
        .then(response => response.json())
        .then(data => {
            console.log(data)
            this.setState({dataRequestGet: data})
        });
    }
    ...
    removeDataRequest() {
        const getIdUbahInvoice = localStorage.getItem("ubahInvoice")
        const { dataTagihan, dataRequestGet } = this.state
        ...
        fetch(`http://localhost:8080/api/invoice/${getIdUbahInvoice}`, {
            method: "PUT",
        ...
        let data = {
            name: dataRequestGet.name,
            division: dataRequestGet.division,
            total: dataRequestGet.total,
            tanggal: dataRequestGet.tanggal,
            keterangan: "Done"
        }

        fetch(`http://localhost:8080/api/request/${dataTagihan[0].idrequest}`, {
            method: "PUT",
        ...
        window.location.href = "http://localhost:5000/invoice"
    }
        ...
        return (
            <Container fluid className="main-content-container px-4 pb-4">
                ...
                <Row noGutters className="page-header py-4">
                    <PageTitle sm="4" title="Invoice" subtitle="Detail Invoice" className="text-sm-left" />
                </Row>
                            ...
                            <CardBody className="p-0 pb-3">
                                    ...
                                    <tbody>
                                        {dataRequest.length > 0 && dataRequest.map((res, idx) => {
                                            return (<tr key={idx}>
                                                ...
                                                <td>
                                                    {
                                                        res.status == "Barang Dikirim" ? (<Button size="sm" theme="warning" className="mb-2 mr-1" onClick={() => this.ModalDelete(res.id,res.idrequest)}>
                                                            View
                                                        </Button>) : ("Done")
                                                    }
                                                </td>
                                ...
                                <Modal open={openModalDelete} toggle={this.ModalDelete}>
                                    <Col md="12" className="form-group">
                                        <ModalHeader>Status Pengiriman</ModalHeader>
                                        <ModalBody>
                                            Apakah barang sudah sampai ditempat ? <br /><br />
                                            <center><button type="submit" className="btn btn-danger" onClick={this.ModalDelete}> Batal</button>&nbsp;
                                                <button type="submit" className="btn btn-primary" onClick={this.removeDataRequest}>Sudah</button></center>
                                        </ModalBody>
                                    </Col>
                                </Modal>
                            </CardBody>
