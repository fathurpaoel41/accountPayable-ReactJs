...
export default class InsertRequest extends Component {
    constructor(props) {
        super(props)
    ...
    componentDidMount() {
        fetch(`http://localhost:8080/api/requestBarang/search?id_request=${this.state.idrequest}`)
    ...
    removeRequestBarang(id){
        ...
        window.location.href = "http://localhost:5000/insert-request"
    }

    onSubmit(e) {
        e.preventDefault()
        localStorage.setItem("statusInsertRequest", "trueRequest")
        console.log({ state: this.state })
        const { nama, divisi, total, tanggal, keterangan, idrequest} = this.state
        let data = {
            name: nama,
            division: divisi,
            total: total,
    ...
    onSubmitRequestBarang(e) {
        e.preventDefault()
        const { idrequest, nama_barang, harga } = this.state
        let data = {
            idrequest: idrequest,
    ...
    render() {
        const { openModalRequestBarang, dataRequestBarang, total } = this.state
        console.log("count total ", this.state.total)
        return (
            <Container fluid className="main-content-container px-4 pb-4">
                {/* Page Header */}
                <Row noGutters className="page-header py-4">
                    <PageTitle sm="4" title="Add Request" subtitle="Detail Edit Request" className="text-sm-left" />
                </Row>
                                ...
                                <Row form>
                                    <Col md="6" className="form-group">
                                        <form onSubmit={this.onSubmit}>
                                            ...
                                            <input type="text" name="nama" className="form-control" placeholder="Masukan Nama Perequest" onChange={this.changeName} required />
                                            <label htmlFor="feEmailAddress">Divisi</label>
                                            <input type="text" name="divisi" className="form-control" placeholder="Masukan Nama Divisi" onChange={this.changeDisivi} />
